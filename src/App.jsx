...
export default function App() {
  ...
  const startLoop = useCallback(() => {
    ...
    const loop = async () => {
      while (isHost.current) {
        /* ── WAITING ── Generate ONE roundId per cycle here */
        ...
        /* ── RUNNING ── Keep the same roundId so placed bets stay valid */
        const cp = randCrash();
        try {
          await updateDoc(GAME_REF, {
            status: "running",
  ...
  /* ───────────────────────────────────────────────────────────
     CASH OUT
     Uses the client-side multiplier at click time as the truth,
     allows a small grace period if the host just wrote "crashed"
     but the player's multiplier was below the crash point.
  ─────────────────────────────────────────────────────────── */
  const doCashOut = async (m) => {
    if (!player) return;
    const cm = parseFloat(m.toFixed(2));
    if (cm < 1.01) return showToast("Multiplier too low", "warn");
    ...
    try {
      const result = await runTransaction(db, async (tx) => {
        ...
        // Validate bet exists for this round
        if (!p.currentBet || p.currentBet <= 0)
          throw new Error("No active bet");
        if (p.betRoundId !== g.roundId)
          throw new Error("Round already ended");
        if (p.cashedOut)
          throw new Error("Already cashed out");

        // Allow cash-out if game is running OR just crashed but our
        // multiplier was below the crash point (grace period race fix)
        const running  = g.status === "running";
        const graceWin = g.status === "crashed" && cm < g.crashPoint;

        if (!running && !graceWin)
          throw new Error(`Too late — crashed at ${g.crashPoint.toFixed(2)}×`);

        // Calculate payout & profit
        const payout = Math.floor(p.currentBet * cm);
        const profit = payout - p.currentBet;

        tx.update(pRef(pid), {
          jCoin: p.jCoin + payout,
          cashedOut: true,
          cashoutMultiplier: cm,
        });

        return { bet: p.currentBet, payout, profit, cm };
      });

      showToast(
        `🎉 Won ${result.payout.toLocaleString()} jCoins at ${fmt(result.cm)} · +${result.profit.toLocaleString()} profit!`,
        "ok",
      );
    } catch (e) {
      ...
      <section className="game-card">
        ...
        {cashed && (
          <div className="ride-info cashed-info">
            ✅ Won {Math.floor(player.currentBet * (player.cashoutMultiplier ?? 1)).toLocaleString()} jCoins @ {fmt(player.cashoutMultiplier ?? 1)}
            &nbsp;·&nbsp;
